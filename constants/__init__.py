"""This module provides constants."""

from constants.assistant_info import (
    ASSISTANT_INFO_TABLE_DATA,
    ASSISTANT_INFO_TABLE_HEADERS,
    ADDRESSBOOK_INFO_TABLE_DATA,
    ADDRESSBOOK_INFO_TABLE_HEADERS,
    NOTEBOOK_INFO_TABLE_DATA,
    NOTEBOOK_INFO_TABLE_HEADERS,
)

__all__ = ["ASSISTANT_INFO_TABLE_DATA",
           "ASSISTANT_INFO_TABLE_HEADERS", "ADDRESSBOOK_INFO_TABLE_DATA", "ADDRESSBOOK_INFO_TABLE_HEADERS", "NOTEBOOK_INFO_TABLE_DATA", "NOTEBOOK_INFO_TABLE_HEADERS"]
